...
the agent edit its own LLM context: fold (collapse) finished or bulky message
ranges into reversible stubs, and recover them (expand / peek / search). This
...
a whole-context "map" to plan batch collapses from, and result numbers it can
trust and cross-check.

## Baseline (current state)

- Tools: `collapse`, `expand`, `peek`, `search`.
- `peek` is overloaded: no arg -> fold overview; `id` -> one fold's members.
- Batching is inconsistent: `collapse`/`expand` take `items: [...]`; `peek`
  takes an optional single `id`; `search` takes a single `query`.
- All prompt encouragement sits on `collapse` (its `promptSnippet` + 5
  `promptGuidelines`). `peek`/`search`/`expand` carry only a `description`.
  Search is mentioned only indirectly, inside collapse's recovery guideline.
- Reported-number bugs (all confirmed in code):
  ...
  - **B — asymmetric definitions.** `collapse.freedTokens` is *net*
    (`Σ(live − stub)`, excludes already-folded members); `expand.restoredTokens`
    is *gross* (`Σ member tokens`, ignores the removed stub and remnant stubs).
    Collapsing then expanding the same range reports different magnitudes.
...
`context_map`, `context_peek`, `context_search`, `context_collapse`,
`context_expand`.
...
`context_collapse`. Backed by a pure core fn `buildContextMap(msgs, spans)`
...
- `context_collapse` / `context_expand`: keep `items: [...]`.

Rationale: consistency, "always batch", make illegal states unrepresentable.

### 4. Dedup the shared preamble

Move the marker mechanics, the fold model, the `map -> search -> peek -> expand`
...
- collapse: `Δlive = stub_new − Σ(newly hidden live members)` (excludes
  already-folded members) -> negative = freed.
- expand: `Δlive = Σ(restored members) − stub_removed + Σ(remnant stubs)` ->
  positive = added.

Rendering:

- Head: `freed |Δlive|` (collapse) / `restored |Δlive|` (expand). Sign derived
  from the value, so no double minus (fixes C).
- Guard: collapse with `Δlive >= 0` -> "no net saving (stub/summary >= hidden
...
fold's own stub keeps an inline `[#id]`, as the handle to expand that exact fold.
...
**Cost:** the first collapse in a session now needs a preceding `context_map`
call to learn ids (the intended orient-then-collapse flow anyway), and id
selection is by map snippet+role+position rather than an inline anchor.

Revision 2 extended this to fold stubs (initially kept as the handle to "expand
what I just saw"): that rationale weakened once map-first became the mandated
flow and `context_collapse`'s own result printed the fold ids anyway, so the
...
results (map/search/peek/collapse).
...
`promptGuidelines` on `context_collapse` carry the policy (when to collapse on
your own, what to keep live, what a summary contains, and the
map → collapse → search → peek → expand flow). Parameter descriptions state
...
`context_expand` could never restore — i.e. "current context", "whole
...
### 9. Reject cross-fold expand ranges

`context_expand` clamped a `to` belonging to another fold, silently restoring
...
guidelines collapse to four (DRY), keeping concrete triggers and examples for
...
- **core.ts**: add `buildContextMap(msgs, spans)` (pure, returns ordered rows for
  live msgs + folds). Change `planExpand` to return net `restoredTokens`
  (`Δlive`) alongside `restoredMsgs`. Keep `planCollapse.freedTokens` (already
...
- **core.test.ts**: update expand assertions to the net `restoredTokens`
  semantics; add a `buildContextMap` test; add batched `peek(ids)` /
  `search(queries)` shape tests.

## Verification

`nix build .#checks.x86_64-linux.pi-extensions` (tsgo + oxlint + node --test)
must stay green; it gates store-mode packaging.
...
"steer" })` pointing at `context_map` + `context_collapse`. It steers only on a
